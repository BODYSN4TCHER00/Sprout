...
import IntervaloDeTiempo from '../components/GestorTiempo'; // Importa tu selector de intervalo de tiempo
import Estadisticas from '../components/stats'; // Importa el componente Estadisticas
...
const TemperaturaScreen = () => {
  ...
  // Función para obtener solo los días de cada fecha
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = response.data.map(item => item.temperatura);
      setChartData({
        labels,
        datasets: [{
          data,
